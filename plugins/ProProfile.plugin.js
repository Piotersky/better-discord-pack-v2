        ...
        ProProfile() {
            ...
            document.addEventListener("dblclick", ({ target }) => {
                if (new RegExp(/guild*/).test(target.dataset.listItemId)) {
                    global.ZLibrary.DiscordModules.ElectronModule.copy(target.children[0].currentSrc.replace(/([0-9]+)$/, "4096"));
                    return BdApi.showToast(`Server icon link successfully copied`, { type: "success" });
                } else if (new RegExp(/animatedBannerHoverLayer*/).test(target.className)) {
                    global.ZLibrary.DiscordModules.ElectronModule.copy(`https://cdn.discordapp.com/banners/${target.__reactFiber$.return.memoizedProps.guild.id}/${target.__reactFiber$.return.memoizedProps.guildBanner}.gif?size=4096`);
                    return BdApi.showToast(`Server banner link successfully copied`, { type: "success" });
                } else if (new RegExp(/height:*/).test(target.style.cssText)) {
                    var strGuildBanner = document.getElementsByClassName("animatedContainer-2laTjx");
                    global.ZLibrary.DiscordModules.ElectronModule.copy(strGuildBanner[0].children[0].children[0].currentSrc.replace(/([0-9]+)$/, "4096"));
                    return BdApi.showToast(`Server icon link successfully copied`, { type: "success" });
                }
            });
